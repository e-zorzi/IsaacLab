...
def reset_root_state_from_list(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor,
    ...
    * It samples the root velocity from the given ranges and sets them into the physics simulation.
    ...
    orientations = math_utils.quat_mul(root_states[:, 3:7], orientations_delta)
    # velocities
    range_list = [((0.0, 0.0)) for _ in ["x", "y", "z", "roll", "pitch", "yaw"]]
    ranges = torch.tensor(range_list, device=asset.device)
    rand_samples = math_utils.sample_uniform(ranges[:, 0], ranges[:, 1], (len(env_ids), 6), device=asset.device)
...
def reset_root_state_from_fixed_list(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor,
    ...
    * It samples the root velocity from the given ranges and sets them into the physics simulation.
    ...
    ``(min, max)``. If the dictionary does not contain a key, the position or velocity is set to zero for that axis.
    """
    global _RNG_FOR_RESET
    if _RNG_FOR_RESET is None:
        _RNG_FOR_RESET = np.random.default_rng(seed)
    # extract the used quantities (to enable type-hinting)
    asset: RigidObject | Articulation = env.scene[asset_cfg.name]
    # get default root state
    root_states = asset.data.default_root_state[env_ids].clone()
    candidate = fixed_list[_RNG_FOR_RESET.integers(0, len(fixed_list))]
    x, y, z = candidate
    pose_range = {"x": (x, x), "y": (y, y), "z": (z, z)}
    # poses
    range_list = [pose_range.get(key, (0.0, 0.0)) for key in ["x", "y", "z", "roll", "pitch", "yaw"]]
